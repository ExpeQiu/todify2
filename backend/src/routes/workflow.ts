...
interface FormattedInput {
  searchResults?: {
    query?: string;
    results?: Array<{ title?: string; content?: string; [key: string]: unknown }>;
  };
  query?: string;
  selectedKnowledgePoints?: Array<{ title?: string; content?: string; [key: string]: unknown }>;
  template?: string;
  [key: string]: unknown;
}

function formatAdditionalInformation(inputs: FormattedInput): string {
  try {
    ...
    if (inputs.searchResults) {
      ...
      if (searchData.results && Array.isArray(searchData.results)) {
        formattedInfo += '搜索结果：\n';
        searchData.results.forEach((result, index: number) => {
    ...
    if (inputs.query || inputs.selectedKnowledgePoints) {
      ...
      if (inputs.selectedKnowledgePoints && Array.isArray(inputs.selectedKnowledgePoints)) {
        formattedInfo += '相关知识点：\n';
        inputs.selectedKnowledgePoints.forEach((kp, index: number) => {
    ...
    Logger.exception(error as Error, 'formatAdditionalInformation');
...
router.post('/ai-search', async (req, res) => {
  try {
    ...
    if (inputs.selectedKnowledgePoints && Array.isArray(inputs.selectedKnowledgePoints) && inputs.selectedKnowledgePoints.length > 0) {
      try {
        Logger.debug('开始处理知识点内容拼接', {
          selectedKnowledgePoints: inputs.selectedKnowledgePoints
        });
        ...
        Logger.error('知识点内容拼接失败', {
          error: contentError instanceof Error ? contentError.message : '未知错误'
        });
    ...
    try {
      ...
      Logger.debug('AI搜索消息已保存到数据库');
    } catch (saveError) {
      Logger.error('保存AI搜索消息到数据库失败', {
    ...
    if (!responseValidation.isValid) {
      Logger.warn('AI搜索响应格式验证失败', { errors: responseValidation.errors });
    }

    // 返回标准化响应
    res.json(formatApiResponse(true, result, 'AI搜索完成'));
  } catch (error) {
    Logger.exception(error as Error, 'AI搜索API');
...
router.post('/tech-package', async (req, res) => {
  try {
    ...
    try {
      ...
      Logger.debug('技术包装消息已保存到数据库');
    } catch (saveError) {
      Logger.error('保存技术包装消息到数据库失败', {
        error: saveError instanceof Error ? saveError.message : '未知错误'
      });
      // 不影响主流程，继续执行
    }
    ...
    if (!responseValidation.isValid) {
      Logger.warn('技术包装响应格式验证失败', { errors: responseValidation.errors });
    }

    res.json(formatApiResponse(true, result, '技术包装完成'));
  } catch (error) {
    Logger.exception(error as Error, '技术包装API');
    res.status(500).json(formatApiResponse(
      false,
      null,
      '技术包装失败',
...
router.post('/tech-strategy', async (req, res) => {
  try {
    ...
    try {
      ...
      Logger.debug('技术策略消息已保存到数据库');
    } catch (saveError) {
      Logger.error('保存技术策略消息到数据库失败', {
        error: saveError instanceof Error ? saveError.message : '未知错误'
      });
      // 不影响主流程，继续执行
    }
    ...
    const responseValidation = validateTechAppResponse(result);
    if (!responseValidation.isValid) {
      Logger.warn('技术策略响应格式验证失败', { errors: responseValidation.errors });
    }

    res.json(formatApiResponse(true, result, '技术策略完成'));
  } catch (error) {
    Logger.exception(error as Error, '技术策略API');
    res.status(500).json(formatApiResponse(
      false,
      null,
      '技术策略失败',
...
router.post('/tech-article', async (req, res) => {
  try {
    ...
    try {
      ...
      Logger.debug('技术通稿消息已保存到数据库');
    } catch (saveError) {
      Logger.error('保存技术通稿消息到数据库失败', {
        error: saveError instanceof Error ? saveError.message : '未知错误'
      });
      // 不影响主流程，继续执行
    }
    ...
    const responseValidation = validateTechAppResponse(result);
    if (!responseValidation.isValid) {
      Logger.warn('技术通稿响应格式验证失败', { errors: responseValidation.errors });
    }

    res.json(formatApiResponse(true, result, '技术通稿完成'));
  } catch (error) {
    Logger.exception(error as Error, '技术通稿API');
    res.status(500).json(formatApiResponse(
      false,
      null,
      '技术通稿失败',
...
router.post('/core-draft', async (req, res) => {
  try {
    ...
    try {
      ...
      Logger.debug('核心稿件消息已保存到数据库');
    } catch (saveError) {
      Logger.error('保存核心稿件消息到数据库失败', {
        error: saveError instanceof Error ? saveError.message : '未知错误'
      });
      // 不影响主流程，继续执行
    }
    ...
    const responseValidation = validateTechAppResponse(result);
    if (!responseValidation.isValid) {
      Logger.warn('核心稿件响应格式验证失败', { errors: responseValidation.errors });
    }

    res.json(formatApiResponse(true, result, '核心稿件生成完成'));
  } catch (error) {
    Logger.exception(error as Error, '核心稿件API');
    res.status(500).json(formatApiResponse(
      false,
      null,
      '核心稿件生成失败',
...
router.post('/speech-generation', async (req, res) => {
  try {
    ...
    Logger.debug('发布会稿生成输入', { speechInputs });
    ...
    try {
      ...
      Logger.debug('发布会稿消息已保存到数据库');
    } catch (saveError) {
      Logger.error('保存发布会稿消息到数据库失败', {
        error: saveError instanceof Error ? saveError.message : '未知错误'
      });
      // 不影响主流程，继续执行
    }
    ...
    const responseValidation = validateTechAppResponse(result);
    if (!responseValidation.isValid) {
      Logger.warn('发布会稿响应格式验证失败', { errors: responseValidation.errors });
    }

    res.json(formatApiResponse(true, result, '发布会稿生成完成'));
  } catch (error) {
    Logger.exception(error as Error, '发布会稿生成API');
    res.status(500).json(formatApiResponse(
      false,
      null,
      '发布会稿生成失败',
...
router.post('/tech-publish', async (req, res) => {
  try {
    ...
    try {
      ...
      Logger.debug('技术发布消息已保存到数据库');
    } catch (saveError) {
      Logger.error('保存技术发布消息到数据库失败', {
        error: saveError instanceof Error ? saveError.message : '未知错误'
      });
      // 不影响主流程，继续执行
    }
    ...
    const responseValidation = validateTechAppResponse(result);
    if (!responseValidation.isValid) {
      Logger.warn('技术发布响应格式验证失败', { errors: responseValidation.errors });
    }

    res.json(formatApiResponse(true, result, '技术发布完成'));
  } catch (error) {
    Logger.exception(error as Error, '技术发布API');
    res.status(500).json(formatApiResponse(
      false,
      null,
      '技术发布失败',
