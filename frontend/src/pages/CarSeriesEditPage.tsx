...
const CarSeriesEditPage: React.FC = () => {
  ...
  const [editingCarModel, setEditingCarModel] = useState<KeyCarModel | null>(
    null,
  );
  ...
  const handleAddCompetitor = (competitor: Competitor) => {
    if (editingCompetitor) {
      setCompetitors((prev) =>
        prev.map((c) => (c === editingCompetitor ? competitor : c)),
      );
                        ...
                        <div className="flex-1">
                          ...
                          {competitor.comparison_points.length > 0 && (
                            <div className="mt-2">
                              ...
                              <div className="flex flex-wrap gap-1">
                                {competitor.comparison_points.map(
                                  (point, idx) => (
                                    <span
                                      key={idx}
...
const CarModelModal: React.FC<CarModelModalProps> = ({
  carModel,
  onSave,
  onCancel,
}) => {
...
const CompetitorModal: React.FC<CompetitorModalProps> = ({
  competitor,
  onSave,
  onCancel,
}) => {
