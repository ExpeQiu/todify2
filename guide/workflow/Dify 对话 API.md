...
## [上传文件](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#files-upload)

上传文件并在发送消息时使用，可实现图文多模态理解。 支持您的应用程序所支持的所有格式。 _上传的文件仅供当前终端用户使用。_
...
curl -X POST 'http://47.113.225.93:9999/v1/files/upload' \
...
## [停止响应](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#Stop)
...
curl -X POST 'http://47.113.225.93:9999/v1/chat-messages/:task_id/stop' \
...
## [消息反馈（点赞）](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#feedbacks)

消息终端用户反馈、点赞，方便应用开发者优化输出预期。

### Path Params

- Name`message_id`TypestringDescription
...
- Name`rating`TypestringDescription
...
- Name`content`TypestringDescription
...
curl -X POST 'http://47.113.225.93:9999/v1/messages/:message_id/feedbacks \
--header 'Authorization: Bearer {api_key}' \
--header 'Content-Type: application/json' \
--data-raw '{
    "rating": "like",
...
## [获取APP的消息点赞和反馈](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#app-feedbacks)

获取应用的终端用户反馈、点赞。

### Query

- Name`page`TypestringDescription
    
- （选填）分页，默认值：1
    
- Name`limit`TypestringDescription
...
GET
/app/feedbacks
curl -X GET 'http://47.113.225.93:9999/v1/app/feedbacks?page=1&limit=20'
```
...
## [获取下一轮建议问题列表](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#suggested)
...
curl --location --request GET 'http://47.113.225.93:9999/v1/messages/{message_id}/suggested?user=abc-123 \
...
## [获取会话历史消息](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#messages)
...
- Name`first_id`TypestringDescription
    
- 当前页第一条聊天记录的 ID，默认 null
    
- Name`limit`TypeintDescription
...
curl -X GET 'http://47.113.225.93:9999/v1/messages?user=abc-123&conversation_id=' \
...
## [获取会话列表](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#conversations)
...
- Name`last_id`TypestringDescription
...
- Name`sort_by`TypestringDescription
...
curl -X GET 'http://47.113.225.93:9999/v1/conversations?user=abc-123&last_id=&limit=20'\
...
## [删除会话](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#delete)
...
curl -X DELETE 'http://47.113.225.93:9999/v1/conversations/:conversation_id' \
--header 'Authorization: Bearer {api_key}' \
--header 'Content-Type: application/json' \
--data-raw '{
 "user": "abc-123"
...
## [会话重命名](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#rename)
...
- Name`name`TypestringDescription
    
- （选填）名称，若 `auto_generate` 为 `true` 时，该参数可不传。
    
- Name`auto_generate`TypeboolDescription
...
curl -X POST 'http://47.113.225.93:9999/v1/conversations/:conversation_id/name' \
...
## [获取对话变量](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#conversation-variables)
...
curl -X GET 'http://47.113.225.93:9999/v1/conversations/{conversation_id}/variables?user=abc-123' \
...
## [语音转文字](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#audio)
...
curl -X POST 'http://47.113.225.93:9999/v1/audio-to-text' \
...
## [文字转语音](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#audio)

文字转语音。

### Request Body

- Name`message_id`TypestrDescription
    
- Dify 生成的文本消息，那么直接传递生成的message-id 即可，后台会通过 message_id 查找相应的内容直接合成语音信息。如果同时传 message_id 和 text，优先使用 message_id。
    
- Name`text`TypestrDescription
...
POST
/text-to-audio
curl -o text-to-audio.mp3 -X POST 'http://47.113.225.93:9999/v1/text-to-audio' \
--header 'Authorization: Bearer {api_key}' \
--header 'Content-Type: application/json' \
--data-raw '{
...
## [获取应用基本信息](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#info)
...
GET
/info
curl -X GET 'http://47.113.225.93:9999/v1/info' \
-H 'Authorization: Bearer {api_key}'
```
...
## [获取应用参数](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#parameters)
...
GET
/parameters
 curl -X GET 'http://47.113.225.93:9999/v1/parameters'\
--header 'Authorization: Bearer {api_key}'
```
...
## [获取应用Meta信息](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#meta)
...
POST
/meta
curl -X GET 'http://47.113.225.93:9999/v1/meta' \
-H 'Authorization: Bearer {api_key}'
```
...
## [获取应用 WebApp 设置](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#site)
...
POST
/meta
curl -X GET 'http://47.113.225.93:9999/v1/site' \
-H 'Authorization: Bearer {api_key}'
```
...
## [获取标注列表](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#annotation_list)
...
GET
/apps/annotations
curl --location --request GET 'http://47.113.225.93:9999/v1/apps/annotations?page=1&limit=20' \
--header 'Authorization: Bearer {api_key}'
```
...
## [创建标注](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#create_annotation)

### Query

- Name`question`TypestringDescription
    
- 问题
    
- Name`answer`TypestringDescription
...
POST
/apps/annotations
curl --location --request POST 'http://47.113.225.93:9999/v1/apps/annotations' \
--header 'Authorization: Bearer {api_key}' \
--header 'Content-Type: application/json' \
...
## [更新标注](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#update_annotation)
...
curl --location --request PUT 'http://47.113.225.93:9999/v1/apps/annotations/{annotation_id}' \
--header 'Authorization: Bearer {api_key}' \
--header 'Content-Type: application/json' \
--data-raw '{"question": "What is your name?","answer": "I am Dify."}'
```
...
## [查询标注回复初始设置任务状态](http://47.113.225.93:9999/app/a541096e-d0c1-48f8-98af-df593cf62428/develop#initial_annotation_reply_settings_task_status)
...
- Name`job_id`TypestringDescription
...
GET
/apps/annotations
curl --location --request GET 'http://47.113.225.93:9999/v1/apps/annotation-reply/{action}/status/{job_id}' \
--header 'Authorization: Bearer {api_key}'
```
